# namespaceclass-controller - AI Agent Guide
...
**After any change under `config/`** (manual edits, or the output of `make manifests`):
Regenerate the Helm chart — see "Option 2: Helm Chart" under Distribution Options below. The chart at `dist/chart/` does not update itself; skipping this leaves it out of sync with `config/`.
...
**Important:** The chart is generated once, not kept in sync automatically. Whenever `config/` changes — a new API/webhook, an edited RBAC/CRD marker, or a manual edit — regenerate it:
