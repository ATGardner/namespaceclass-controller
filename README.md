...
**Prerequisite:** the webhooks (both the Helm chart's and the plain YAML
bundle's below) need TLS certs, provisioned via
...
NamespaceClass definition. Any change to the label value (swithcing between
...
The admission webhook validates that every resource defined in a NamespceClass
...
cluster-scoped resouce in its `spec.resources`, and the mutating-webhook makes
sure a client can't change any of the managed fields defined in the
NamespaceClass. in case one or the other fails (or even disabled during the
